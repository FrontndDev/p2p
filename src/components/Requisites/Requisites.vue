  ...
  <div class="requisites p2p-card">
    ...
    <div class="requisites__content">
      ...
      >
        ...
        <div class="requisites__item-card-num">{{ censuredRequisite(requisite.requisite) }}</div>
...
const censuredRequisite = (requisite: string) => {
  if (requisite.length < 7) return '*'?.repeat(requisite.length)

  // Извлечение первых и последних 3 символов
  const firstPart = requisite?.slice(0, 3);
  const lastPart = requisite?.slice(-3);
  // Создание средней части строки с *
  const middlePart = '*'?.repeat(requisite.length - 6);
